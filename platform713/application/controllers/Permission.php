...
class Permission extends CI_Controller{
	...
	public function permissionfacelist(){
		...
		//得到总条数
		$this->load->model('Permission_model');
		$total = $this->Permission_model->getAccessFaceListTotal($village_id,$effective_date,$person_type,$building_code,$keyword,$this->user_per_page);
		$totals = $this->Permission_model->getApplyFaceTotal($village_id,$effective_date,$person_type,$building_code,$keyword,$this->user_per_page);
	...
	public function applyfacelist(){
		...
		//得到总条数
		$this->load->model('Permission_model');
		$total = $this->Permission_model->getApplyFaceListTotal($village_id,$effective_date,$person_type,$building_code,$keyword,$this->user_per_page);
	...
	public function refusefacelist(){
		...
		$now = date("Y-m-d",time());
		//得到今天的30天以前
		$month_early = date('Y-m-d',time()-30*3600*24);
		$start_date = $start_date?$start_date:$month_early;
		...
		//得到总条数
		$this->load->model('Permission_model');
		$total = $this->Permission_model->getRefuseFaceListTotal($village_id,$start_date,$end_date,$person_type,$building_code,$keyword,$this->user_per_page);
		$total = 1;

		//得到申请人脸总条数
		$totals = $this->Permission_model->getApplyFaceTotal($village_id,'',$person_type,$building_code,$keyword,$this->user_per_page);

		$data['page']=$page>=$total?$total:$page;
		$data['total']=$total;
		$data['totals']=$totals;
		$data['keyword']=$keyword;
		$data['effective_date']=$start_date;
	...
	public function getRefuseFaceList(){
		$page = $this->input->get('page');
		$keyword = $this->input->get('keyword');
		$start_date = $this->input->get('start_date');
		$end_date = $this->input->get('end_date');
		...
		$end_date = $end_date?$end_date:$now;
		$this->load->model('Permission_model');
		$res = $this->Permission_model->getRefuseFaceList($village_id,$start_date,$end_date,$person_type,$building_code,$keyword,$page,$this->user_per_page);
		echo $res;
	}
	...
	public function insertAccessFace(){
		...
		//得到总页数
		$data['total'] = $this->Permission_model->getApplyFaceListTotal($village_id,$search_effective_date,$search_person_type,$search_building_code,$search_keyword,$this->user_per_page);
		//得到申请总条数
		$data['totals'] = $this->Permission_model->getApplyFaceTotal($village_id,$search_effective_date,$search_person_type,$search_building_code,$search_keyword,$this->user_per_page);
		print_r(json_encode($data));
	}
	...
	public function updateAccessFace(){
		...
		//得到总条数
		$data['total'] = $this->Permission_model->getAccessFaceListTotal($village_id,$search_effective_date,$search_person_type,$search_building_code,$search_keyword,$this->user_per_page);
		print_r(json_encode($data));
	}
	...
	public function refuseApplyFace(){
		...
		$this->load->model('Permission_model');
		$now = date('Y-m-d',time());
		$res = $this->Permission_model->refuseApplyFace($village_id,$code,$reason,102,$now,$_SESSION['person_code']);	
		//最后判断是否全部写入成功
		if($res==true){
			$data['message'] = '编辑成功';
		}
