...
$('#add_Item .confirm').click(function(){
    ...
    //人员信息
    var person_code =  $('.add_Item').find("option:selected").attr('data-code');
if(person_code==undefined)
{
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if(data){
                ...
                for(var i=0;i<data.length;i++){
                    var d = data[i];
                    var html=html+ ' <option data-subtext='+d['code']+' data-code='+d['code']+' >'+d['full_name']+'</option>';
                }
