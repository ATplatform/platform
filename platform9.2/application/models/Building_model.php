    ...
    public function getBuildingTreeData(){
        ...
        if(!empty($root_tree)){
         ...
         /*   $node=$root_tree;
            ...
            foreach($node as $k => $v){
                $row=$this->treeALLnode($v);
                $node=$row;
            }

            }*/
            $root_tree= $this->treeALLnode($root_tree);
            return json_encode($root_tree);
        }
        return false;
    }

    public function treeALLnode($node){
        $res = $this->getTreeNodeByPcode($node['code']);
        if(!empty($res)){
        $node['children']=$res;

        foreach($res as $k => $v){
            $node['children'][$k]= $this->treeALLnode($v);
