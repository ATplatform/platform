   ...
   var heightnow= $('#tree svg').attr("height")
    if (d.children) {
        ...
        if( height==0 &&  (!d.children && d._children)){
            console.log('ttttt')
            $('#tree svg').attr("height", h)
            tree = d3.layout.tree().size([h, w]);
        }
        if( height!=0 &&  (!d.children && d._children)){
            $('#tree svg').attr("height", h+height)
            tree = d3.layout.tree().size([h+height, w]);
        }
        }else{
            $('#tree svg').attr("height", h)
            tree = d3.layout.tree().size([h, w]);
...
function locate(d){
    ...
    if(height==0 && !d.parent.children.children )
    {
        $('#tree svg').attr("height", h)
        tree = d3.layout.tree().size([h, w]);
    }
    ...
    if(height!=0 && (!d.children || !d._children)){
        $('#tree svg').attr("height", height)
        tree = d3.layout.tree().size([height, w]);
    }
