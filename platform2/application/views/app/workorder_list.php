...
<!--<?php echo 'x'; ?>-->
     ...
     <!--   <a id="sendSelect" href="javascript:;">yes</a>-->
        ...
        <form class="search_room" action="" method="get">
            <p>
                ...
                <a id="clear" href="<?=base_url().'index.php/Workorder/workorderList'?>">X</a>
            </p>
    ...
    <!-- 物资数据-->
        ...
        <ul class="pager" page='<? $page ?>'>
            <?php
            $first=base_url().'index.php/Workorder/workorderList?page=1&parent_code='.$parent_code.'&building_code='.$building_code.'&material_type='.$material_type.'&keyword='.$keyword.'&effective_date='.$effective_date;
            echo  " <li><a href='".$first."' id='first'>首 页</a></li>";
            if($page>1) {
                $url=base_url().'index.php/Workorder/workorderList?page='.($page-1).'&parent_code='.$parent_code.'&building_code='.$building_code.'&material_type='.$material_type.'&keyword='.$keyword.'&effective_date='.$effective_date;
            ...
            $last=base_url().'index.php/Workorder/workorderList?page='.$total.'&parent_code='.$parent_code.'&building_code='.$building_code.'&material_type='.$material_type.'&keyword='.$keyword.'&effective_date='.$effective_date;
    ...
    //////////////////////////////搜索模块的树形地点///////////////////////////////////
       ...
       // console.log(treeNav_data);
        ...
        $('#treeNav>span').on("select_node.jstree", function (e, node) {
           ...
           window.location.href="workorderList?building_code="+building_code+"&parent_code="+parent_code+"&page="+page+'&material_type='+material_type+'&keyword='+keyword+'&effective_date='+effective_date;
        })
