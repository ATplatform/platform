...
class Contract_model extends CI_Model
{
    ...
    public function sqlTogetWorkorderList($begin_date,$type,$level, $keyword, $page, $rows)
    {
        $start = ($page - 1) * $rows;


         /////////////////判断为普通查询或搜索查询////////////////////////////
         if (empty($parent_code) && empty($building_code) && empty($type) && empty($keyword) && empty($begin_date) && empty($level))
...
";
            ...
            foreach ($arr as $key => $value) {
                foreach ($value as $key2 => $value2) {
                    ...
                    if($key2 == 'position_code'){
                        $arr[$key]['position_name']=$arr[$key]['p_name'];
                    }
                   /* if ($key2 == 'process_pic'){
    ...
    //////////////////////搜索查询数据数目的数据总条数/////////////
    public function getWorkorderListTotal($begin_date,$parent_code, $building_code, $type, $keyword, $level,$rows)
    {
