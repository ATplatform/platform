...
class Message extends CI_Controller{
	...
	public function messagelist(){
		...
		$total = $this->Message_model->getMessageListTotal($village_id,$push_start_date,$push_end_date,$msg_type,$cycle_type,$keyword,$this->user_per_page);
		// echo $total;exit;
		...
		$data['village_id'] = $_SESSION['village_id'];
	...
	public function getMessageCode(){
		$village_id = $_SESSION['village_id'];
		$this->load->model('Message_model');
		$res = $this->Message_model->getMessageCode($village_id);
		if(empty($res)){
			$res = 1000000;
		}
		echo $res;
	...
	public function getMessageList(){
		...
		//写入数据
		$res = $this->Message_model->getMessageList($village_id,$push_start_date,$push_end_date,$msg_type,$cycle_type,$keyword,$page,$this->user_per_page);
		echo $res;
	}
	...
	public function messagerecordlist(){
		...
		//得到信息条目总数
		$this->load->model('Message_model');
		$total = $this->Message_model->getMessageRecordListTotal($village_id,$if_has_receipt,$push_state,$push_start_date,$push_end_date,$msg_type,$cycle_type,$keyword,$this->user_per_page);
		...
		$data['if_has_receipt']=$if_has_receipt;
		$data['pagesize']=$this->user_per_page;
	...
	public function getmessagerecordlist(){
		...
		//写入数据
		$res = $this->Message_model->getMessageRecordList($village_id,$if_has_receipt,$push_state,$push_start_date,$push_end_date,$msg_type,$cycle_type,$keyword,$page,$this->user_per_page);
		echo $res;
	}
	...
	public function getMessageTarget(){
		...
		foreach($target as $key => $row){
			...
			//102表示推送给设备
			elseif($target_type==102){
				$this->load->model('Equipment_model');
				$equipment = $this->Equipment_model->getEquipmentByCode($row,$village_id);
				$res[$key]['household'] = $equipment['name'];
			}
	...
	public function updateMessage(){
		...
		$village_id = $_SESSION['village_id'];
		$this->load->model('Message_model');
		$res = $this->Message_model->updateMessage($code,$push_end_date,$village_id);
		if($res==true){
			$data['message'] = '编辑消息成功';
		}
		else {
		...
		//得到数据总页数
		$data['total'] = $this->Message_model->getMessageListTotal($village_id,$search_push_start_date,$search_push_end_date,$search_msg_type,$search_cycle_type,$search_keyword,$this->user_per_page);
		print_r(json_encode($data));
	}
}
