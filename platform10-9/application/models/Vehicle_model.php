...
public function updateVehicle($code,$effective_date,$effective_status
...
public function updateAuz($code,$vehicle_code,$begin_date
...
public function updateParkinglot($code,$effective_date,$effective_status,$linked_lot_code,$begin_date,$end_date,$area,$monthly_rent,$parkcode,$floor,$biz_type,$biz_status,$biz_reason,$owner,$remark){
    ...
    public function sqlTogetvehiclepkg($effective_date,$parkcode,$floor,$biz_type,$biz_status,$biz_reason,$keyword, $page, $rows)
    {
        $start = ($page - 1) * $rows;
...
";
    ...
    public function getvehiclepkg($sql)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            foreach ($arr as $key => $value) {
                foreach ($value as $key2 => $value2) {
                    if ($key2 == 'pkg_vehicle_code') {
                        $arr[$key]['pkg_vehicle_code_name'] = $value2;
                    }
                    if ($key2 == 'pkg_licence') {
                        $arr[$key]['pkg_licence_name'] = $value2;
                    }
                    if ($key2 == 'pkg_entry_eqp') {
                        $arr[$key]['pkg_entry_eqp_name'] = $value2;
                    }
                    if ($key2 == 'pkg_entry_type') {
                        $arr[$key]['pkg_entry_type_name'] = $value2;
                    }
                     if ($key2 == 'pkg_exit_eqp') {
                          $arr[$key]['pkg_exit_eqp_name'] = $value2;
                      }
                    if ($key2 == 'pkg_exit_type') {
                        $arr[$key]['pkg_exit_type_name'] = $value2;
                    }

                    if ($key2 == 'pkg_charge_amount') {
                        $arr[$key]['pkg_charge_amount_name'] = $value2;
                    }
                    if ($key2 == 'pkg_related_id') {
                           $arr[$key]['pkg_related_id_name'] = $value2;
                       }

                    if ($key2 == 'pkg_parking_payment_id') {
                        $arr[$key]['pkg_parking_payment_id_name'] = $value2;
                    }
