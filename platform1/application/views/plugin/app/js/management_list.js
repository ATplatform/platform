...
$('#relation_detail .save').click(function(){
	...
	var person_code = add_relation.find('.person_code').data('ajax');
	var hire_date = add_relation.find('.hire_date').html();
	var begin_date = add_relation.find('.begin_date').html();
	var employee_no = add_relation.find('.employee_no').html();
	var remark = add_relation.find('.remark').html();
	//必填项
	var position_name = add_relation.find('.position_name').val();
	var end_date = add_relation.find('.end_date').val();
	var p_bs = add_relation.find('.select_buliding em');
			...
			success:function(message){
				...
				for(var i=0;i<data.length;i++){
					...
					if($("#relation_detail .select_buliding #"+id).length==0){
						$('.select_buliding').append('<em id='+id+' data-room_code='+code+'>'+household+'<i class="fa fa-close"></i></em>');
					}
				}
			},
			error:function(jqXHR,textStatus,errorThrown){
