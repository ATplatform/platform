...
class People_model extends CI_model {
	...
	public function updatePeople($code,$if_disabled,$oth_mob_no,$remark){
		if(is_null($oth_mob_no)||empty($oth_mob_no)){
			$oth_mob_no = 'null';
		}
		$sql = "UPDATE village_person SET if_disabled = '$if_disabled',oth_mob_no = '$oth_mob_no',remark = '$remark'  WHERE code = '$code' ";
		$query = $this->db->query($sql);
		return $this->db->affected_rows();
	}
	...
	public function getResidentList($if_disabled,$birth_begin,$birth_end,$household_type,$effective_date,$keyword,$page,$rows){
		...
		$sql = "select *,concat(p.last_name,p.first_name) as full_name,pb.remark as pb_remark from village_person_building as pb LEFT JOIN village_person as p on pb.person_code = p.code  LEFT JOIN village_tmp_building as bs on pb.building_code = bs.room_code where  ";
		//筛选条件,有筛选日期
	...
	public function getResidentListTotal($if_disabled,$birth_begin,$birth_end,$household_type,$effective_date,$keyword,$rows){
		$sql = "select count(*) as count from village_person_building as pb LEFT JOIN village_person as p on pb.person_code = p.code  LEFT JOIN village_tmp_building as bs on pb.building_code = bs.room_code  where ";
		//筛选条件,有筛选日期
		if(!empty($effective_date)){
			$sql .= " pb.begin_date <= '$effective_date' and pb.end_date >= '$effective_date' ";
		}
	...
	public function insertPersonBiz($building_code,$begin_date,$end_date,$biz_info,$remark,$person_code,$household_type,$create_time){
		//先查出最新的code;
		...
		$insert_sql = "INSERT INTO village_person_biz (code,building_code,person_code,begin_date,end_date,biz_type,biz_info,remark,create_time) values ($code,$building_code,$person_code,'$begin_date','$end_date',$household_type,'$biz_info','$remark','$create_time')";
		// echo $insert_sql;exit;
		$this->db->query($insert_sql);
		return $this->db->affected_rows();
	}

	public function getBusinessList($effective_date,$building_code,$biz_type,$keyword,$page,$rows){
		$gender_arr=$this->gender_arr;
		$id_type_arr=$this->id_type_arr;
		$blood_type_arr=$this->blood_type_arr;
		$ethnicity_name_arr=$this->ethnicity_name_arr;
		...
		$sql = "SELECT *, concat (P .last_name, P .first_name) AS full_name,pb.remark AS pb_remark,p.remark as p_remark,pb.code as biz_code FROM village_person_biz AS pb LEFT JOIN village_person AS P ON pb.person_code = P .code LEFT JOIN village_tmp_building AS bs ON pb.building_code = bs.room_code LEFT JOIN village_building as b on pb.building_code = b.code WHERE effective_date =(select max(effective_date) from village_building b_i where b.name = b_i.name and b.parent_code = b_i.parent_code and b_i.effective_status = true ) and ";
		//筛选条件,有筛选日期
		if(!empty($effective_date)){
			$sql .= " pb.begin_date <= '$effective_date' and pb.end_date >= '$effective_date' ";
		}
		...
		if(!empty($building_code)){
			$sql .= " and b.code = $building_code or b.parent_code = $building_code ";
		}

		//搜索条件 1楼宇名称 2姓名 3经营内容
		if(!empty($keyword)){
			$sql .= " and concat(name,immeuble,last_name,first_name,pb.biz_info) like '%$keyword%' ";
		}
	...
	public function getBusinessListTotal($effective_date,$biz_type,$building_code,$keyword,$rows){
		$sql = "select count(*) as count FROM village_person_biz AS pb LEFT JOIN village_person AS P ON pb.person_code = P .code LEFT JOIN village_tmp_building AS bs ON pb.building_code = bs.room_code LEFT JOIN village_building as b on pb.building_code = b.code WHERE effective_date =(select max(effective_date) from village_building b_i where b.name = b_i.name and b.parent_code = b_i.parent_code and b_i.effective_status = true ) and ";
		...
		//搜索条件 1楼宇名称 2姓名 3经营内容
		if(!empty($keyword)){
			$sql .= " and concat(room,immeuble,last_name,first_name,pb.biz_info) like '%$keyword%' ";
		}
		// echo $sql;exit;
		$q = $this->db->query($sql); //自动转义
		if ( $q->num_rows() > 0 ) {
	...
	public function updatePersonBiz($biz_code,$end_date,$remark,$biz_info){
		$sql = "UPDATE village_person_biz SET end_date = '$end_date',biz_info = '$biz_info',remark = '$remark'  WHERE code = '$biz_code' ";
		$query = $this->db->query($sql);
		return $this->db->affected_rows();
	}

	public function updatePersonPosition($code,$end_date,$remark,$position_code){
		$sql = "UPDATE village_person_position SET end_date = '$end_date',position_code = '$position_code',remark = '$remark'  WHERE code = '$code' ";
		// echo $sql;exit;
		$query = $this->db->query($sql);
		return $this->db->affected_rows();
	}
