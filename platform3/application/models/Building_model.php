...
class Building_model extends CI_Model {
    ...
    public function getBuildingByCode($code){
        $sql = "select id,code,name,effective_date,stage,area,immeuble,unit,floor,room  from village_building LEFT JOIN village_tmp_building on village_building.code = village_tmp_building.room_code where code = $code limit 1";
        // echo $sql;
        // echo "<br />";
        $query = $this->db->query($sql);
        $row = $query->row_array();
    ...
    public function getBuildingLast(){
        $sql = "SELECT code,parent_code,effective_date,NAME FROM village_building a WHERE effective_status = TRUE and effective_date = (select max(effective_date) from village_building b where a.name = b.name and a.parent_code = b.parent_code and b.effective_status = true ) ORDER BY code,name";
        $q = $this->db->query($sql);
        if ( $q->num_rows() > 0 ) {
            $arr=$q->result_array();
            $json=json_encode($arr);
    ...
    public function getBuilding($code){
        $sql = "select * from village_tmp_building where room_code = $code limit 1";
        $query = $this->db->query($sql);
        $row = $query->row_array();
        return $row;
    }
