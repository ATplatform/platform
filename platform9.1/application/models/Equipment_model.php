...
class Equipment_model extends CI_model {
		...
		if ( $q->num_rows() > 0 ) {
			...
			foreach($arr as $key => $row){
				...
				if(!empty($row['building_code'])){
					...
					//根据拼接成的结果查出所有的地点信息
					foreach($building_code_arr as $k2 => $v2){
						$building = $this->getBuildingByCode($v2);
						$household = $this->getHouseholdInfo($building);
				...
				if(!empty($row['person_code'])){
					...
					foreach($person_code_arr as $k2 => $v2){
						...
						array_push($arr[$key]['person_name_arr'],$name);
						array_push($arr[$key]['person_id_arr'],$id_number);
					}
				}
			}
			$json=json_encode($arr);
	...
	public function getPersoneEquipmentListTotal($keyword,$effective_date,$equipment_type,$building_code,$rows){
