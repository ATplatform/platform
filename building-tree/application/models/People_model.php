...
class People_model extends CI_model {
	...
	public function getBusinessList($effective_date,$building_code,$biz_type,$keyword,$page,$rows){
		...
		$sql = "SELECT *, concat (P .last_name, P .first_name) AS full_name,pb.remark AS pb_remark,p.remark as p_remark,pb.code as biz_code FROM village_person_biz AS pb LEFT JOIN village_person AS P ON pb.person_code = P .code LEFT JOIN village_building_stage AS bs ON pb.building_code = bs.room_code LEFT JOIN village_building as b on pb.building_code = b.code WHERE effective_date =(select max(effective_date) from village_building b_i where b.name = b_i.name and b.parent_code = b_i.parent_code and b_i.effective_status = true ) and ";
		//筛选条件,有筛选日期
		if(!empty($effective_date)){
			$sql .= " pb.begin_date <= '$effective_date' and pb.end_date >= '$effective_date' ";
		}
	...
	public function getBusinessListTotal($effective_date,$biz_type,$building_code,$keyword,$rows){
		$sql = "select count(*) as count FROM village_person_biz AS pb LEFT JOIN village_person AS P ON pb.person_code = P .code LEFT JOIN village_building_stage AS bs ON pb.building_code = bs.room_code LEFT JOIN village_building as b on pb.building_code = b.code WHERE effective_date =(select max(effective_date) from village_building b_i where b.name = b_i.name and b.parent_code = b_i.parent_code and b_i.effective_status = true ) and ";
