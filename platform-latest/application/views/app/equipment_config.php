	...
	<div class="col-sm-12 main_wrap">
		<div class="searc_bar search_wrap" id="search_wrap">
			...
			<div class="search_room msg_search_room">
				<p>
					<input type="text" class="searc_room_text" name="keyword" placeholder="可输入设备名称、服务器IP、公网IP、局域网IP" value="">
...
$(function(){
	...
	$('.fenye_btn').click(function(){
		...
		window.location.href="equipmentlist?keyword="+search_keyword+"&page="+page+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
	})
	//生效日期筛选
	$('.search_wrap .effective_date').datetimepicker().on('changeDate', function(ev){
        var date = $('.effective_date').val();
        window.location.href="equipmentlist?keyword="+search_keyword+"&page=1"+"&effective_date="+date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
    })

    //树节点点击筛选
    $('#treeNav>span').on("select_node.jstree", function (e, node) {
      var building_code = node.node.original.code;
      window.location.href="equipmentlist?keyword="+search_keyword+"&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+building_code;
    })

	//设备类型筛选
	$('.equipment_type_wrap .first_nav,.equipment_type_wrap .subNav li').click(function(){
		var equipment_type = $(this).find('a').data('ajax');
		window.location.href="equipmentlist?keyword="+search_keyword+"&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+equipment_type+'&building_code='+search_building_code;
	})
	//巡检周期筛选
	$('.regular_check_wrap .ka_drop_list li').click(function(){
		var regular_check = $(this).find('a').data('ajax');
		window.location.href="equipmentlist?keyword="+search_keyword+"&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
	})
	...
	$('.search_room button[type="submit"]').click(function(){
		...
		window.location.href="equipmentlist?keyword="+keyword+"&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
	})
	//清除条件
