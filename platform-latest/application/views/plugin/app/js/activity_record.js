    ...
    return   {
       ...
       ,
        ...
        findinsert:function(){
            ...
            var person_code_length=$('.person_building_data li').length
            if(!this.insert.must.code) {openLayer('请输入活动编号');return;}
            if(!this.insert.must.service_code) {openLayer('请输入物业负责人');return;}
            if(person_code_length==0) {openLayer('请输入参与的业主');return;}
            if(!this.insert.must.date) {openLayer('请输入活动日期');return;}
...
var router={
    ...
    getactivity_codeUrl:getRootPath()+'/index.php/Activity/getactivity_codeUrl',
}
...
$('.add_Item').find('input[name="date"]').val(init_time);
...
search(List,'date','type','keyword')
insert(router.getLatestCode,router.getservice_code,router.insert,router.getactivity_codeUrl,List)
...
/////////////////////////////////////////////展示////////////////////////////////////////////
            ...
            /*////////////////////////////////////////附件处理//////////////////////////////////////////////////
                        $('#person_detail').find('.additional' ).html('<a download="file" href='+rowkeys.additional + ' class="col_37A" >'+rowkeys.additional.split('Contracts/')[1]+'</a>');*/
...
function insert(){
   ...
   var getactivity_codeUrl=arguments[3];
    ...
    $.ajax({
        ...
        success:function(message){
            ...
            for(var i=0;i<data.length;i++){
                ...
                var a_name =d['a_name'];
                var a_code=d['a_code']
                if($(".select_room #"+a_code).length==0){
                    $('.select_activity ul').append('<li><a href="javascript:;" id='+a_code+' data-ajax='+a_code+'>'+a_name+'-'+a_code+'</a></li>');
                }
            }
        },
        ...
        success:function(message){
            ...
            for(var i=0;i<data.length;i++){
                ...
                if($(".select_room #"+pp_code).length==0){
                    $('.select_person ul').append('<li><a href="javascript:;" id='+pp_code+' data-ajax='+pp_code+'>'+pp_name+'</a></li>');
                }
            }
        },
        error:function(jqXHR,textStatus,errorThrown){
...
PageChange.index.method=router.List
urlParam= PageChange.index
