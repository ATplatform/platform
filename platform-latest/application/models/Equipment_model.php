...
class Equipment_model extends CI_model {
	...
	public function getEquipmentByName($name,$village_id){
		$sql = "select e.code,e.name,e.building_code,b.stage_name,b.area_name,b.immeuble_name,b.unit_name,b.floor_name,b.room_name,b.public_name,b.room from village_equipment as e LEFT JOIN village_tmp_building as b on e.building_code = b.room and e.village_id = b.village_id  where e.name like '%$name%'  and e.effective_status = true and e.village_id = $village_id ";
		$q = $this->db->query($sql); //自动转义
		if ( $q->num_rows() > 0 ) {
			$arr=$q->result_array();
			foreach($arr as $key => $row){
	...
	public function getEquipmentConfig($village_id,$page,$keyword,$effective_date,$equipment_type,$building_code,$rows){
		...
		$effective_status_arr=$this->effective_status_arr;
		$annual_check_arr=$this->annual_check_arr;
		$sql = "select b.stage_name,b.area_name,b.immeuble_name,b.unit_name,b.floor_name,b.room_name,b.public_name,bd.parent_code as building_parent_code,e.name as e_name,e.parent_code as e_parent_code,e.code as code,e.village_id,e.building_code,e.server_ip,e.lan_ip,e.ip,e.gateway,e.netmask,e.dns1,e.dns2,e.sip,e.tdcode_url from village_equipment as e LEFT JOIN village_tmp_building as b on e.building_code = b.room and e.village_id = b.village_id  left join village_building as bd on e.building_code = bd.code  and e.village_id = bd.village_id ";
		$sql=$sql." where bd.effective_date = (select max(effective_date) from village_building b_i where bd.name = b_i.name and bd.parent_code = b_i.parent_code and b_i.effective_status = true  and b_i.effective_date <= now() ) ";
		$sql=$sql." and e.effective_status = true ";
	...
	public function getEquipmentConfigTotal($village_id,$page,$keyword,$effective_date,$equipment_type,$building_code,$rows){
		...
		$effective_status_arr=$this->effective_status_arr;
		$annual_check_arr=$this->annual_check_arr;
		$sql = "select count(e.code) as count from village_equipment as e LEFT JOIN village_tmp_building as b on e.building_code = b.room and e.village_id = b.village_id left join village_building as bd on e.building_code = bd.code AND e.village_id = bd.village_id ";
		$sql=$sql." where bd.effective_date = (select max(effective_date) from village_building b_i where bd.name = b_i.name and bd.parent_code = b_i.parent_code and b_i.effective_status = true  and b_i.effective_date <= now() ) ";
		$sql=$sql." and e.effective_status = true ";
		$sql=$sql." and e.equipment_type in (301,302,303,304,305,306,307) ";
		$sql=$sql." and e.server_ip !='' ";
