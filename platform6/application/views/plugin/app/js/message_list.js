...
$('#add_message .view').click(function(){
	...
	if($('.if_cycle_wrap input[type="radio"]').eq(0).is(':checked')){
		...
		if_cycle = 103;
		cycle_type = $('.create_type_wrap .cycle_type').data('ajax');
		push_end_date = $('.create_type_wrap .push_end_date').val();
		if(!cycle_type){
			openLayer('请选择循环周期!');
			return;
		}
...
$('#confirm_message .add,#confirm_message .view').click(function(){
	$.ajax({
		...
		success:function(message){
			...
			//添加完成之后跳转页面
			var now = getDate();
			window.location.href="messagelist?page=1"+'&push_start_date='+now+' 00:00 &push_end_date='+now+" 23:59";
		}
	})
...
window.operateEvents = {
	...
	'click .detail':function(e,value,row,index){
		...
		$('#message_detail').find('.bill_amount em').html(bill_amount);
	...
	'click .rewrite':function(e,value,row,index){
		...
		$('#message_write').find('.bill_amount em').html(bill_amount);
