             ...
             if(!empty($level)){
               ...
               if (!empty($keyword)) {
                   if (preg_match('/^\d*[\x7f-\xff]+\d*$/', $keyword)) {
                       $sql .= " and concat ( p_name,c.signed_with,c.remark) like '%$keyword%'";
                   }
...
public function insert($code,$type,$level,$signed_with,$amount, $position_code,$begin_date,$end_date,$remark,$create_time)
{

    $sql = " INSERT INTO village_contracts (code,type,level,signed_with,amount, position_code,begin_date,end_date,remark,create_time) values (".
