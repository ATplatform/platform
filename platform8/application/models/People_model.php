...
class People_model extends CI_model {
	...
	public function getResidentList($building_code,$level,$if_disabled,$birth_begin,$birth_end,$household_type,$effective_date,$keyword,$page,$rows){
	...
	public function getResidentListTotal($building_code,$level,$if_disabled,$birth_begin,$birth_end,$household_type,$effective_date,$keyword,$rows){
		$sql = "select count(*) as count from village_person_building as pb LEFT JOIN village_person as p on pb.person_code = p.code  LEFT JOIN village_tmp_building as bs on pb.building_code = bs.room  where pb.begin_date = (select max(begin_date) from village_person_building as b where  pb.building_code = b.building_code and pb.person_code = b.person_code  and b.begin_date <= now() )  ";
		//筛选条件,有筛选日期
		if(!empty($effective_date)){
			$sql .= " and pb.begin_date <= '$effective_date' and pb.end_date >= '$effective_date' ";
		}
	...
	public function getBusinessList($effective_date,$building_code,$level,$biz_type,$keyword,$page,$rows){
		$gender_arr=$this->gender_arr;
		$id_type_arr=$this->id_type_arr;
		$blood_type_arr=$this->blood_type_arr;
		$ethnicity_name_arr=$this->ethnicity_name_arr;
		...
		$sql = "SELECT *, concat (P .last_name, P .first_name) AS full_name,pb.remark AS pb_remark,p.remark as p_remark,pb.code as biz_code FROM village_person_biz AS pb LEFT JOIN village_person AS P ON pb.person_code = P .code LEFT JOIN village_tmp_building AS bs ON pb.building_code = bs.code  WHERE ";
		//筛选条件,有筛选日期
		if(!empty($effective_date)){
	...
	public function getBusinessListTotal($effective_date,$biz_type,$building_code,$level,$keyword,$rows){
		$sql = "select count(*) as count FROM village_person_biz AS pb LEFT JOIN village_person AS P ON pb.person_code = P .code LEFT JOIN village_tmp_building AS bs ON pb.building_code = bs.code  WHERE  ";
	...
	public function getBuildingByCode($code){
	    $sql = "select * from village_tmp_building where code = '$code' ";
	    // echo $sql;
	    // echo "<br />";
	    $query = $this->db->query($sql);
	    $row = $query->row_array();
