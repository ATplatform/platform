...
class Building_model extends CI_Model {
    ...
    public function getBuildingsList($keyword,$id,$parent_code,$page,$rows){
        $start=($page-1) * $rows;
    	$sql = "select id,village_building.code,effective_date,effective_status,name,level,rank,parent_code,remark from village_building";
        $limit = false;
        if(!empty($keyword)){
            if($limit){
        ...
        if(!empty($id)||!empty($parent_code)){
            if(!empty($keyword)){
              ...
              if($limit){
                  $sql .= " and id = $id or parent_code = $parent_code";
              }
              else{
    ...
    public function getBuildingTotal($keyword,$id,$parent_code,$rows){
