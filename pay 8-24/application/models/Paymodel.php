...
class Paymodel extends CI_Model
{
    ...
    {
        $sql="select * from village_bill_list where ( pay_status=101 or pay_status=102 ) and (bill_type=101 or bill_type=103 or bill_type=104 or bill_type=105)";
        return $sql;
    }
    ...
    {
        $sql="select * from village_parking_lot where code=$bill_source_code and village_id=$village_id";
        $q = $this->db->query($sql);
        $arr = $q->result_array();
        return  $arr;
    }
...
where pb.person_code=$person_code and pb.village_id=$village_id and pb.id=(select max(id) from village_person_building A where A.building_code=pb.building_code ) ";
   ...
   public function get_all_property_record($person_code,$village_id)
    ...
    public function get_car_list($person_code,$village_id)
    {
        $sql="select * from village_vehicle where person_code=$person_code and village_id=$village_id ";
        $q = $this->db->query($sql);
        $arr = $q->result_array();
    ...
    public function get_park_lot_name($park_lot,$village_id){
        $sql="select parkname from village_park where village_code=$village_id and parkcode=$park_lot";
        $q = $this->db->query($sql);
        $arr = $q->row_array();
    ...
    public function get_village_name($village_id){
        $sql="select full_name from village_village where id=$village_id";
        $q = $this->db->query($sql);
        $arr = $q->row_array();
    ...
    public function  get_park_lot($village_id){
        $sql="select parkcode,parkname from village_park where village_code=$village_id";
        $q = $this->db->query($sql);
        $arr = $q->result_array();
