...
class Material extends CI_Controller{
    ...
    public function getMaterialListbyNormal(){
        ...
        $dataNormal = $this->Material_model->getMaterialListbyNormal($page,$this->user_per_page);
        echo $dataNormal;
    }
    ...
    public function getMaterialListbySearch(){
        ...
        $page = $page?$page:'1';
        $this->load->model('Material_model');
        $dataSearch = $this->Material_model->getMaterialListbySearch($parent_code,$building_code,$material_type,$keyword,$page,$this->user_per_page);
        echo $dataSearch;
    }
...
//完美的函数，下面不用看
 ...
 //数据展示
    ...
    public function insertMaterial(){
        ...
        if($res){$data['message'] = '新增物资成功';}
        else {$data['message'] = '新增物资失败';}
        $total=$this->Material_model->getMaterialTotalbyNormal($this->user_per_page );
        $data['total'] = $total;
