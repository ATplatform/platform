	...
	<div class="col-sm-12 main_wrap">
		...
		<div class="table_wrap">
			<div class="oh pt10">
				...
				<a class="fr add_btn" href="<?=base_url().'index.php/Equipment/personequipmentlist'?>">清除筛选</a>
			</div>
		...
		<ul class="pager" page='<? $page ?>'>
		    <?php
		       $first=base_url().'index.php/Equipment/personequipmentlist?page=1&keyword='.$keyword.'&effective_date='.$effective_date.'&equipment_type='.$equipment_type.'&building_code='.$building_code;	
		       echo  " <li><a href='".$first."' id='first'>首 页</a></li>";
		    if($page>1) {
					$url=base_url().'index.php/Equipment/personequipmentlist?page='.($page-1).'&keyword='.$keyword.'&effective_date='.$effective_date.'&equipment_type='.$equipment_type.'&building_code='.$building_code; 
		    ...
		    $last=base_url().'index.php/Equipment/personequipmentlist?page='.$total.'&keyword='.$keyword.'&effective_date='.$effective_date.'&equipment_type='.$equipment_type.'&building_code='.$building_code;
					...
					<div class="choose_equip_building">
						...
						<div style="padding-left: 90px;margin-bottom:10px;" class="select_buliding_wrap">
							...
							<span class="select_buliding">
								<!-- <em id="100507" data-room_code="100507">111期111区111栋711单元211层123<i class="fa fa-close"></i></em> -->
							</span>
						</div>
...
$(function(){
	...
	$('#table').bootstrapTable({
		method: "get",
		undefinedText:' ',
		url:getRootPath()+'/index.php/Equipment/getPersoneEquipmentList?page='+page+'&keyword='+search_keyword+'&effective_date='+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code,
		dataType:'json',
	...
	$('.fenye_btn').click(function(){
		...
		window.location.href="personequipmentlist?keyword="+search_keyword+"&page="+page+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
	})
	//生效日期筛选
	$('.search_wrap .effective_date').datetimepicker().on('changeDate', function(ev){
        var date = $('.effective_date').val();
        window.location.href="personequipmentlist?keyword="+search_keyword+"&page=1"+"&effective_date="+date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
    })

    //树节点点击筛选
    $('#treeNav>span').on("select_node.jstree", function (e, node) {
      var building_code = node.node.original.code;
      window.location.href="personequipmentlist?keyword="+search_keyword+"&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+building_code;
    })

	//设备类型筛选
	$('.equipment_type_wrap .first_nav,.equipment_type_wrap .subNav li').click(function(){
		var equipment_type = $(this).find('a').data('ajax');
		window.location.href="personequipmentlist?keyword="+search_keyword+"&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+equipment_type+'&building_code='+search_building_code;
	})
	...
	$('.search_room #clear').click(function(){
		...
		window.location.href="personequipmentlist?keyword=&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
	})
