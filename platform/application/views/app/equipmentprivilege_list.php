	...
	<div class="col-sm-12 main_wrap">
		...
		<div class="table_wrap">
			<div class="oh pt10">
				...
				<a class="fr add_btn" href="<?=base_url().'index.php/Equipment/equipmentprivilegelist'?>">清除筛选</a>
			</div>
			...
			<thead>
				<tr>
					...
					<th data-field="pe_remark" data-title="备注" data-align="center" data-formatter="viewAll"></th>
		...
		<ul class="pager" page='<? $page ?>'>
		    <?php
		       $first=base_url().'index.php/Equipment/equipmentprivilegelist?page=1&keyword='.$keyword.'&effective_date='.$effective_date.'&equipment_type='.$equipment_type.'&building_code='.$building_code;	
		       echo  " <li><a href='".$first."' id='first'>首 页</a></li>";
		    if($page>1) {
					$url=base_url().'index.php/Equipment/equipmentprivilegelist?page='.($page-1).'&keyword='.$keyword.'&effective_date='.$effective_date.'&equipment_type='.$equipment_type.'&building_code='.$building_code; 
		    ...
		    $last=base_url().'index.php/Equipment/equipmentprivilegelist?page='.$total.'&keyword='.$keyword.'&effective_date='.$effective_date.'&equipment_type='.$equipment_type.'&building_code='.$building_code;
	            ...
	            <div class="modal-body building">
					...
					<div class="search_person_wrap search_equipment_wrap">
						<div class="oh" style="margin-bottom:10px;">
							...
							<div class="fl search_person_text" style="margin-left:14px;">
					...
					<p><span class="red_star">*</span>开始日期：
						<input type="text" class="begin_date date form-control" name="begin_date" placeholder="请输入开始日期" >
					</p>

					<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;结束日期：
						<input type="text" class="end_date date form-control" name="end_date" placeholder="请输入结束日期">
					</p>
...
$(function(){
	...
	$('#table').bootstrapTable({
		method: "get",
		undefinedText:' ',
		url:getRootPath()+'/index.php/Equipment/getPrivilegeEquipmentList?page='+page+'&keyword='+search_keyword+'&effective_date='+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code,
		dataType:'json',
	...
	$('.fenye_btn').click(function(){
		...
		window.location.href="equipmentprivilegelist?keyword="+search_keyword+"&page="+page+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
	})
	//生效日期筛选
	$('.search_wrap .effective_date').datetimepicker().on('changeDate', function(ev){
        var date = $('.effective_date').val();
        window.location.href="equipmentprivilegelist?keyword="+search_keyword+"&page=1"+"&effective_date="+date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
    })

    //树节点点击筛选
    $('#treeNav>span').on("select_node.jstree", function (e, node) {
      var building_code = node.node.original.code;
      window.location.href="equipmentprivilegelist?keyword="+search_keyword+"&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+building_code;
    })

	//设备类型筛选
	$('.equipment_type_wrap .first_nav,.equipment_type_wrap .subNav li').click(function(){
		var equipment_type = $(this).find('a').data('ajax');
		window.location.href="equipmentprivilegelist?keyword="+search_keyword+"&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+equipment_type+'&building_code='+search_building_code;
	})
	...
	$('.search_room button[type="submit"]').click(function(){
		...
		window.location.href="equipmentprivilegelist?keyword="+keyword+"&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
	})
	//清除条件
	$('.search_room #clear').click(function(){
		window.location.href="equipmentprivilegelist?page=1";
		window.location.href="equipmentprivilegelist?keyword=&page=1"+"&effective_date="+search_effective_date+'&equipment_type='+search_equipment_type+'&building_code='+search_building_code;
	})
...
<script src='<?=base_url().'application/views/plugin/app/js/equipmentprivilege_list.js'?>'></script>
</body>
</html>