...
class Material_model extends CI_Model
{
    ...
    public function getMaterialList($building_code, $material_type, $keyword, $page, $rows)
    {
        ...
        $sql = "SELECT * FROM village_material AS M LEFT JOIN village_building_stage AS bs ON M .building_code = bs.room_code LEFT JOIN village_building AS b ON M .building_code = b.code WHERE
   b.effective_date = (
		SELECT
			MAX (effective_date)
		FROM
		...
		WHERE
			b. NAME = A . NAME
		AND b.parent_code = A .parent_code
		AND A .effective_status = TRUE
	)
AND b.effective_status = TRUE";
        ...
        if (!empty($keyword)) {
            if (preg_match("/^\d*$/", $keyword)) {
                $sql .= " and code=$keyword or material_type=$keyword";
            }
            if (preg_match('/^\d*[\x7f-\xff]+\d*$/', $keyword)) {
                $sql .= " and concat(supplier,remark,function) like '%$keyword%'";
            }
            }
...
//判断keyword是数字还是汉字
    ...
    public function getMaterialTotal($parent_code,$building_code, $material_type, $keyword, $rows)
    {
...
AND b.effective_status = TRUE";
        ...
        if (!empty($material_type)) {
            ...
            if (!empty($parent_code)){
                $sql .= " or parent_code=$parent_code";
            }
        }
            if (!empty($keyword)) {
