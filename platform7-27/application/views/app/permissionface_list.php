...
<!--<?php echo 'x'; ?>-->
	<div class="col-sm-12 main_wrap">
		...
		<div class="table_wrap">
			<div class="oh pt10">
				<span class="sub_nav" style="display: block;">
					<a class="active" href="<?=base_url().'index.php/Permission/permissionfacelist'?>">人脸识别授权列表</a>
					...
					<a class="fr add_btn" href="<?=base_url().'index.php/Permission/permissionfacelist'?>">清除筛选</a>
				</span>
			</div>
			...
			<thead>
				<tr>
					...
					<th data-field="code" data-title="人脸识别授权编号" data-align="center"></th>
		...
		<ul class="pager" page='<? $page ?>'>
		    <?php
		       $first=base_url().'index.php/Permission/permissionfacelist?page=1&keyword='.$keyword.'&effective_date='.$effective_date.'&person_type='.$person_type.'&building_code='.$building_code;	
		       echo  " <li><a href='".$first."' id='first'>首 页</a></li>";
		    if($page>1) {
					$url=base_url().'index.php/Permission/permissionfacelist?page='.($page-1).'&keyword='.$keyword.'&effective_date='.$effective_date.'&person_type='.$person_type.'&building_code='.$building_code; 
		    ...
		    $last=base_url().'index.php/Permission/permissionfacelist?page='.$total.'&keyword='.$keyword.'&effective_date='.$effective_date.'&person_type='.$person_type.'&building_code='.$building_code;
        ...
        <div class="modal-content model_wrap">
        	<div class="model_content">
	            ...
	            <div class="modal-body building oh">
		            ...
		            <p class="select_buliding_wrap" style="line-height: 30px;"><span class="des">授权地址：</span>
		            ...
		            <p><span class="des">Face来源：</span>
		            	<span class="source_type_name col_37A"></span>
		            </p>
	            </div>
        	</div>
        ...
        <div class="modal-content model_wrap">
            ...
            <div class="modal_footer bg_eee">
            	<p class="tac pb17">
                	...
                	<span class="col_FFA refuse" data-dismiss="modal">取消</span>
            	</p>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal -->
...
$(function(){
	...
	$('#table').bootstrapTable({
		method: "get",
		undefinedText:' ',
		url:getRootPath()+'/index.php/Permission/getAccessFaceList?page='+page+'&keyword='+search_keyword+'&effective_date='+search_effective_date+'&person_type='+search_person_type+'&building_code='+search_building_code,
		dataType:'json',
	...
	$('.search_wrap .effective_date').datetimepicker().on('changeDate', function(ev){
        ...
        window.location.href="permissionfacelist?keyword="+search_keyword+"&page=1"+"&effective_date="+date+'&person_type='+search_person_type+'&building_code='+search_building_code;
    })
	//用户类型筛选
	$('.search_wrap .person_type_wrap .ka_drop_list li').click(function(){
		var person_type = $(this).find('a').data('ajax');
		window.location.href="permissionfacelist?keyword="+search_keyword+"&page=1"+"&effective_date="+search_effective_date+'&person_type='+person_type+'&building_code='+search_building_code;
	})
	//树形菜单点击筛选
	$('#treeNav>span').on("select_node.jstree", function (e, node) {
	  var building_code = node.node.original.code;
	  window.location.href="permissionfacelist?keyword="+search_keyword+"&page=1"+"&effective_date="+search_effective_date+'&person_type='+search_person_type+'&building_code='+building_code;
	})
	//点击搜索按钮,跳转
	$('.search_room button[type="submit"]').click(function(){
		...
		window.location.href="permissionfacelist?keyword="+keyword+"&page=1"+"&effective_date="+search_effective_date+'&person_type='+search_person_type+'&building_code='+search_building_code;
	})
	//清除条件
	$('.search_room #clear').click(function(){
		window.location.href="permissionfacelist?page=1";
		window.location.href="permissionfacelist?keyword=&page=1"+"&effective_date="+search_effective_date+'&person_type='+search_person_type+'&building_code='+search_building_code;
	})
})
</script>
<script>
...
<script src='<?=base_url().'application/views/plugin/app/js/permissionface_list.js'?>'></script>
</body>
</html>